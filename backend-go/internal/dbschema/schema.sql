...
CREATE TABLE public.task_plans (
    ...
    created_at timestamp without time zone DEFAULT now() NOT NULL,
    updated_at timestamp without time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_task_plans_schema_version CHECK (((schema_version)::text = ANY ((ARRAY['agent.task_plan.v1'::character varying, 'agent.task_plan.v2'::character varying])::text[]))),
