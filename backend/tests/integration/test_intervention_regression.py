...
def test_suite_context_rerun_failed_reuses_context_snapshot_after_manual_correction(
    client,
    isolated_artifacts_root: Path,
) -> None:
    _ensure_chromium_available()
    ...
    with serve_static_fixture("intervention_flow") as base_url:
        ...
        consumer_case_id = _create_case(
            client,
            name="读取 landing_url 并完成跳转",
            base_url=base_url,
            input_contract=[
                {
                    "name": "landingUrl",
            ...
            steps=[
                {"action": "goto", "value": "/"},
                {"action": "click", "target": "handoff trigger"},
                {"action": "assert_url_contains", "value": "${landing_url}"},
            ],
        )
        suite_response = client.post(
            ...
            json={
                "project_id": 1,
                "actor_user_id": 1,
                "name": "Suite Context Rerun Fixture",
                "cases": [{"case_id": producer_case_id}, {"case_id": consumer_case_id}],
            },
        )
        assert suite_response.status_code == 201
        suite_id = suite_response.json()["id"]

        initial_run = client.post(f"/api/v1/suites/{suite_id}/execute", json={"actor_user_id": 1})
        assert initial_run.status_code == 200
        initial_payload = initial_run.json()
        assert initial_payload["status"] == "failed"
        assert initial_payload["context_snapshot"] == {"landing_url": f"{base_url}/success.html"}
        assert initial_payload["items"][0]["status"] == "passed"
        assert initial_payload["items"][0]["context_writes"][0]["status"] == "written"
        assert initial_payload["items"][1]["status"] == "needs_intervention"
        assert initial_payload["items"][1]["context_reads"][0]["resolved"] is True
        assert initial_payload["items"][1]["context_reads"][0]["context_key"] == "landing_url"

        failed_execution_id = initial_payload["items"][1]["execution_id"]
        failed_execution = client.get(f"/api/v1/executions/{failed_execution_id}")
        assert failed_execution.status_code == 200
        intervention_request = failed_execution.json()["report"]["steps"][1]["intervention_request"]
        assert intervention_request is not None
        page_url = intervention_request["page_url"]
        assert page_url.startswith(base_url)
        ...
        create_correction = client.post(
            ...
            json={
                ...
                "correction_type": "test_id",
                "correction_value": "fixture-primary-action",
                "source_execution_id": failed_execution_id,
                "created_by": 1,
            },
        )
        assert create_correction.status_code == 201

        initial_run_id = initial_payload["id"]

        rerun_response = client.post(
            f"/api/v1/suites/{suite_id}/runs/{initial_run_id}/rerun-failed",
            json={"actor_user_id": 1},
        )
        assert rerun_response.status_code == 200
        rerun_payload = rerun_response.json()
        assert rerun_payload["status"] == "passed"
        assert rerun_payload["source"] == "rerun_failed"
        assert rerun_payload["context_source"] == "suite_run_snapshot"
        assert rerun_payload["context_snapshot"] == {"landing_url": f"{base_url}/success.html"}
        assert len(rerun_payload["items"]) == 1
        assert rerun_payload["items"][0]["case_id"] == consumer_case_id
        assert rerun_payload["items"][0]["status"] == "passed"
        assert rerun_payload["items"][0]["context_reads"][0]["resolved"] is True
        assert rerun_payload["items"][0]["context_reads"][0]["source_suite_run_id"] == initial_run_id

        rerun_execution_id = rerun_payload["items"][0]["execution_id"]
        rerun_execution = client.get(f"/api/v1/executions/{rerun_execution_id}")
        assert rerun_execution.status_code == 200
        rerun_execution_payload = rerun_execution.json()
        assert rerun_execution_payload["status"] == "passed"
        assert rerun_execution_payload["latest_url"] == f"{base_url}/success.html"
        assert rerun_execution_payload["report"]["steps"][1]["resolved_by"] == "correction:test_id"
        assert rerun_execution_payload["report"]["steps"][2]["value"] == f"{base_url}/success.html"
