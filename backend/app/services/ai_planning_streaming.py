...
async def stream_multi_agent_planning(
    *,
    session_factory: sessionmaker,
    planning_session_id: int,
    content: str,
    ...
    """Bridge multi-agent orchestrator to async SSE events."""
    from app.ai.multi_agent import MultiAgentOrchestrator

    logger.info("Starting multi-agent planning for session %d", planning_session_id)

    def _run_multi_agent(db: Session):
        from app.models import AIPlanningSession, SessionProject
        from sqlalchemy import select

        session_obj = db.get(AIPlanningSession, planning_session_id)
        if not session_obj:
            yield {"type": "error", "message": f"Session {planning_session_id} not found"}
            return

        project_id = 0
        sp = db.execute(
            select(SessionProject).where(SessionProject.session_id == planning_session_id)
        ).scalars().first()
        if sp:
        ...
        yield from orchestrator.run(content)

    async for event in _bridge_sync_generator(
        session_factory=session_factory,
        generator_factory=_run_multi_agent,
        phase="multi_agent",
    ):
        yield event
