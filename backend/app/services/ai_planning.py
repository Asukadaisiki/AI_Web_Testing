...
def retest_cases(
    ...
    *,
    ...
    # --- Self-healing loop: retry with AI-suggested fixes ---
    _MAX_HEAL_RETRIES = 3
    heal_attempt = 0
    while heal_attempt < _MAX_HEAL_RETRIES:
        all_passed = all(s.status == "passed" for s in execution_summaries)
        if all_passed:
            break

        heal_attempt += 1
        logger.info("Self-healing attempt %d/%d", heal_attempt, _MAX_HEAL_RETRIES)
        ...
        if not analysis_response or not analysis_response.execution_analysis:
            break

        # Try to auto-fix failed cases based on AI analysis
        fixes_applied = 0
        for ex_summary in execution_summaries:
            if ex_summary.status == "passed":
                continue
            fixed = _try_auto_fix_case(
                session, ex_summary.case_id, ex_summary.case_name,
                analysis_response,
            )
            if fixed:
                fixes_applied += 1
        ...
        if fixes_applied == 0:
            logger.info("Self-healing: no fixes applied, stopping")
            break

        logger.info("Self-healing: applied %d fixes, re-executing", fixes_applied)
        # Re-execute fixed cases
        execution_summaries = []
        for case_id in case_ids:
            case_record = session.get(TestCase, case_id)
            if case_record is None or (project_ids and case_record.project_id != project_ids[0]):
...
def _try_auto_fix_case(
    session: Session,
    case_id: int,
    case_name: str,
    analysis_response: AIPlanningTurnResponse,
) -> bool:
    """Apply AI-suggested DSL fixes to a failed case.

    Parses execution failure details from the analysis response and attempts
    to fix common locator issues (missing text_parent_chain, CSS→text, etc.)
    ...
    dsl = case_record.dsl.copy()
    steps = dsl.get("steps", [])
    if not steps:
        return False

    analysis = analysis_response.execution_analysis
    if not analysis:
        return False

    fixes_applied = 0
    failure_details = analysis.failure_details or []

    for fd in failure_details:
        step_index = fd.get("step_index", -1)
        suspected = (fd.get("suspected_cause") or "").lower()
        action = fd.get("action", "")
        target = fd.get("target", "") or ""

        if step_index < 0 or step_index >= len(steps):
            continue
        step = steps[step_index]

        # Pattern 1: "No locator found" → rewrite target for text_parent_chain
        if "no locator found" in suspected or "not found" in suspected:
            # Check if there's a nearby product name we can use
            _product_contexts = _find_nearby_product_targets(steps, step_index)
            for ctx in _product_contexts:
                new_target = f"{ctx} 附近的 {target}"
                if new_target != target:
                    step["target"] = new_target
                    logger.info(
                        "Auto-fix case %d step %d: %r → %r",
                        case_id, step_index, target, new_target,
                    )
                    fixes_applied += 1
                    break

        # Pattern 2: CSS selector target → convert to text_parent_chain
        if str(target).startswith("css=") and _product_contexts:
            ctx = _product_contexts[0]
            new_target = f"{ctx} 附近的 {action}"
            step["target"] = new_target
            logger.info(
                "Auto-fix case %d step %d: css→text %r → %r",
                case_id, step_index, target, new_target,
            )
            fixes_applied += 1

        # Pattern 3: "数量" / "数量输入框" → "1" (actual quantity text)
        if "数量" in str(target) and action == "input":
            _new = target.replace("数量输入框", "1").replace("数量", "1").replace("附近的 ", "附近的 ")
            if _new != target:
                step["target"] = _new
                fixes_applied += 1
    ...
    if fixes_applied > 0:
        ...
        logger.info(
            "Auto-fixed case %d (%s): %d step(s) updated",
            case_id, case_name, fixes_applied,
        )
        return True
    return False


def _find_nearby_product_targets(
    steps: list[dict[str, Any]],
    current_idx: int,
) -> list[str]:
    """Find product name targets from nearby capture_text or assert_text steps."""
    products: list[str] = []
    for i in range(max(0, current_idx - 15), min(len(steps), current_idx + 5)):
        s = steps[i] if isinstance(steps[i], dict) else {}
        t = (s.get("target") or "").strip()
        a = s.get("action", "")
        if a in ("capture_text", "assert_text", "click") and t:
            # Only use non-numeric, non-css targets as product context
            if not t.startswith("css=") and not t.isdigit() and len(t) > 2:
                products.append(t)
    return list(dict.fromkeys(products[-3:]))  # dedup, keep last 3
