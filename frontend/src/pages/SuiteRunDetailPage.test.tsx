...
test("批次详情页渲染摘要和子执行链接", async () => {
  vi.mocked(api.getSuiteRunDetail).mockResolvedValue({
    ...
    items: [
      { id: 1, case_id: 11, case_name_snapshot: "提交订单", order_index: 1, execution_id: 101, status: "failed" },
      { id: 2, case_id: 12, case_name_snapshot: "取消订单", order_index: 2, execution_id: 102, status: "passed" },
    ],
  });
  ...
  vi.mocked(api.getSuiteRunDetail).mockImplementation(async (_suiteId, runId) =>
    runId === 7
      ...
      : {
          ...
          items: [
            { id: 3, case_id: 11, case_name_snapshot: "提交订单", order_index: 1, execution_id: 103, status: "passed" },
          ],
        },
  );
