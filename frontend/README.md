...
- 已切换：Planning 工作台使用 Go AgentCore 的 AgentRun、ToolCall、Checkpoint 和 SSE 重放协议；Python API 继续提供会话、项目和浏览器能力
...
开发代理默认将 `/api/v2/agent` 转发到 Go `http://127.0.0.1:8081`，其余 `/api` 和 `/artifacts` 转发到 Python `http://127.0.0.1:8000`。可分别通过 `VITE_AGENTCORE_PROXY_TARGET` 和 `VITE_PYTHON_PROXY_TARGET` 覆盖。
