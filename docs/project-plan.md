...
- 已完成 `单 Case 平台化 v1.8`：新增 `Dashboard` 与独立 `报告中心` 入口，根路由默认进入仪表盘，平台入口从“用例 / 执行明细”扩展为“仪表盘 / 用例 / 执行中心 / 报告中心”。
- `GET /api/v1/executions/overview` 已支持可选 `window_days=7/14/30`，并新增 `trend_points`、`failure_step_actions`、`top_failed_cases` 三类聚合字段。
- 仪表盘已可展示近 7 天执行趋势、最近失败执行和失败最多用例；报告中心已可展示窗口切换、失败分类分布、失败动作分布、高频失败用例与最近失败跳转。
...
- `executions overview` 聚合：支持通过率、平均耗时、最近失败、失败分类、按天趋势、失败动作分布与高频失败用例
- 后端与前端最小测试链路
...
下一里程碑已从“前端可演示闭环 v1”切换为“单 Case 平台化 v1.8”。

- 目标：把现有单 Case 执行链路提升为可观察、可导航、可聚合的平台入口。
- 范围：仪表盘趋势总览、报告中心聚合、执行中心明细、报告详情跳转和高频失败识别。
- 展示原则：继续以截图、结构化证据、趋势聚合和执行详情为主，不做远程浏览器窗口串流。
