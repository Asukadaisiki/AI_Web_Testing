...
当前四层降级链路已跑通最小闭环，P0-P4 已全部在仓库中落地；后续重点从“继续加能力”转为“围绕 AI visual 灰度验收验证收益、命中率与延迟”。
...
- AI visual 默认继续关闭，只在灰度验收口径下评估是否值得进入默认开启讨论
- 2026-03-24 已完成首个本地受控灰度观察窗口：3 条固定浏览器主回归全部通过，但 `ai_visual_stats` 仍未达到有效样本量，因此继续保持默认关闭，不进入默认开启评估
- 继续保留 3 条浏览器固定主回归，作为混合定位与人工干预链路的长期门槛
- 不再为 Locator 方向重开新主线，优先把收益、稳定性和延迟观测做扎实
...
- 执行轨迹可视化
- 失败分类
- ReportPage 前端报告页（项目级执行结果、概览统计、可展开步骤证据）
...
- 用例管理（`Project -> Case` 资产结构，CRUD 完整）
- 2 条浏览器级固定主回归长期保留
